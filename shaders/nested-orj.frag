...
{     
    ...
    {      
        ...
        vec3 texPos = (vec4(position, 1) * matrix).xyz + 0.5; //Figure out where we need to sample the texture from           
    ...
    ds = ds * custom_min(1.0, 1.0 / maxScale);   
   ...
   //The ray     
    ...
    else
    {
        ...
        for(int i = 0; i < numValidTextures; i++)
        {
            ...
            vec3 box_min = (vec4(box_min0, 1) * iMatrix).xyz;
            vec3 box_max = (vec4(box_max0, 1) * iMatrix).xyz;
